...
image_name = "plaidcloud/plaidcloud-utilities"

podTemplate(label: 'plaidcloud-utilities',
  containers: [
...
{
  node(label: 'plaidcloud-utilities') {
    properties([
      parameters([
            ...
            docker.withRegistry("", "plaid-docker") {
              ...
              stage('Run Linter') {
                if (CHANGE_BRANCH == 'master' || params.full_lint) {
                  image.withRun('-t', 'bash -c "pylint plaidcloud.utilities -j 0 -f parseable -r no>pylint.log"') {c ->
                    sh """
                      docker wait ${c.id}
                      docker cp ${c.id}:/home/plaid/src/plaidcloud-utilities/pylint.log pylint.log
                    """
                  }
                } else {
                  image.withRun('-t') {c ->
              ...
              stage('Run Tests') {
                image.withRun("-t", "pytest") {c ->
                  sh """
                    docker wait ${c.id}
                    docker cp ${c.id}:/home/plaid/src/plaidcloud-utilities/pytestresult.xml pytestresult.xml
                    docker cp ${c.id}:/home/plaid/src/plaidcloud-utilities/coverage.xml coverage.xml
                  """
                }
