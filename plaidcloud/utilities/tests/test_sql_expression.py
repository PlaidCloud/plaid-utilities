...
class TestEvalExpressionRaise(unittest.TestCase):
    """Covers the `raise` path in eval_expression when apply_variables fails
    and disable_variables is False."""
...
class TestProcessFnTrim(unittest.TestCase):
    """Covers the trim_fn branch when trim_type is truthy."""
    ...
    def test_trim_true_wraps_rtrim(self):
        ...
        compiled_sql = _compile(result)
        # Expect two rtrim calls in compiled SQL
        self.assertIn('rtrim', compiled_sql.lower())
    ...
    def test_trim_false_is_identity(self):
        ...
        compiled_sql = _compile(result)
        self.assertNotIn('rtrim', compiled_sql.lower())


class TestResultClass(unittest.TestCase):
    """Covers the Result.__init__ dict comprehension."""
...
class TestAllocate(unittest.TestCase):
    ...
    def test_allocate_basic_no_parent_context(self):
        ...
        compiled_sql = _compile(stmt)
        self.assertIn('alloc_source_1', compiled_sql)
        self.assertIn('alloc_driver_1', compiled_sql)
        self.assertIn('consol_driver_1', compiled_sql)
        self.assertIn('denominator_1', compiled_sql)
        self.assertIn('ratios_1', compiled_sql)
    ...
    def test_allocate_with_include_source_columns_and_no_overwrite(self):
        ...
        compiled_sql = _compile(stmt)
        self.assertIn('amount_allocated', compiled_sql)
        self.assertIn('amount_source', compiled_sql)
    ...
    def test_allocate_with_parent_context(self):
        ...
        compiled_sql = _compile(stmt)
        self.assertIn('parent_region_3', compiled_sql)
        self.assertIn('leaves_region_3', compiled_sql)
        self.assertIn('parent_driver_3', compiled_sql)
...
class TestApplyRules(unittest.TestCase):
    ...
    def test_apply_rules_include_once_true(self):
        ...
        compiled_sql = _compile(final_select)
        self.assertIn('rule_id', compiled_sql)
        self.assertIn('applied_rules', compiled_sql)
